...
try:
    stm=serial.Serial("COM3",9600,timeout=0.1)#'COM3' is the port number and it we will know it from the arduino ide or the device manager in the widows
...
status_label=tk.Label(root,text="No Gas detected",font=('Times New Roman',18),fg="green")
status_label.pack(pady=20)
#to monitor temp
temp_label=tk.Label(root,text="Temperature:--°C",font=('Times New Roman',18),fg="black")
...
def record_cry(file_name="cry_record.wav",duration=3,sample_rate=16000):
    try:
        record=sd.rec(int(sample_rate*duration),samplerate=sample_rate,channels=1,dtype="float32")
        sd.wait()
...
def audio_process(audio_file_path):
    ...
    try:
        ...
        status_label.config(text=f"Baby Crying Alert!\nReason: {crying_message}",font=('Times New Roman',22),fg="red")
    ...
    while stm and stm.in_waiting>0:
        try:
            ...
            if receievd_message=="ALERT:GAS DETECTED":
                ...
                status_label.config(text="Baby Awake",font=('Times New Roman',22),fg="red")
            elif receievd_message.startswith("temperature:"): # Matches exact case + colon
